// RoleRevealView.js - swipe-to-reveal role cards
...
export function RoleRevealView(data) {
  ...
  // Replace back button with a styled chevron left
  nav.querySelector('.nav-btn').innerHTML = `
  ...
  function renderLayer(playerIndex, isIncoming = false) {
    ...
    roleLayer.style.background = role.color; // Darken it?
    // Overlay a dark gradient so it looks like iOS
    roleLayer.innerHTML = `
      ...
      <div style="position:relative;z-index:1;display:flex;flex-direction:column;align-items:center;">
        ...
        <div style="font-size:16px;font-weight:600;color:rgba(255,255,255,0.8);margin-top:16px;text-align:center;padding:0 20px;">${role.description}</div>
      </div>
    `;
    layer.appendChild(roleLayer);
    ...
    if (img) {
      ...
      bg.innerHTML = `<div class="fallback-bg"><div class="fallback-initial">${player.name.charAt(0).toUpperCase()}</div></div>`;
    }
  ...
  function render() {
    ...
    if (!isTransitioning) {
      ...
      // Transitioning: show current fixed, and incoming sliding in
      const currentLayer = renderLayer(session.currentPlayerIndex);
  ...
  return { element: el, cleanup: () => { window.removeEventListener('mousemove', onMouseMove); window.removeEventListener('mouseup', onMouseUp); } };
}
